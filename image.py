...
choice = input("""
      This is my personal photo library! There are 10 cool random pictures in this library that you can edit and play around with however youd like! Check it out, and enjoy!
      
      
      Type a number between 1 to 10 to select a photo to open, or type in "quit" to exit. 
      """)
...
if choice.isdigit():
      if ("1" in choice) or ("2" in choice) or ("3" in choice) or ("4" in choice) or ("5" in choice) or ("6" in choice) or ("7" in choice) or ("8" in choice) or ("9" in choice) or ("10" in choice):
            choice1 = str(input("""
            Alright firstly, do you want to save the image as a png instead of jpeg? \n
            Yes or no
            """))
            if choice1 == "yes":
            ...
            choice2 = str(input("""
            You are also able to change the size of the thumbnail of the picture! to change it to 200 type in "200", for 400 type "400",
            for 600 enter "600", but if you dont want to change the size enter "no"
            """))
            if choice2 != "no":
                  size(locals()["img" + choice])
      
            choice3 = str(input("""
            Would you like to rotate the picture?
            Enter yes if your down yo or enter no if your not down.
             """))     
            if choice3 == "yes":
                 rotate(locals()["img" + choice])
            
            choice4 = str(input("""
            Would you like to switch the picture to black and white? yes or no: 
            """))
            if choice4 == "yes":
                  color(locals()["img" + choice])
            
            choice5 = str(input("""
            You can also choose if you want to blur the picture or not
            Do you want to blur it? yes or no: 
            """))
            if choice5 == "yes":
      ...
      if final == "yes":
            ...
            print("Buddy that isn't a number corresponding to a picture, select a number between 1-10")
            choice = input()

elif choice == "quit":
      print("Aight peace out gang!")
      sleep(0.5)
      empty
      quit()
      
else:
      print("Buddy, that is not one of the choices for the picture! Input a number from 1-10 please.")
